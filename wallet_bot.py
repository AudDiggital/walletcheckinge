...
GOOGLE_CREDENTIALS_FILE = "/Users/ceve/Desktop/WalletCheckinge/credentials.json"
...
def emoji_tipo(tipo: str) -> str:
    """Retorna emoji según tipo de transacción."""
    t = tipo.strip().upper()
    if t == "ENVIADO":
        return "🔴➡️  *Enviado*"
    elif t == "RECIBIDO":
...
async def procesar_wallet(update: Update, wallet: str) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Error: No se encontró la hoja de cálculo. Verifica el nombre en la configuración del bot."
        )
    except Exception as e:
        logger.error("Error inesperado: %s", e)
        await update.message.reply_text(
            "❌ Ocurrió un error al consultar la base de datos. Intenta de nuevo en unos segundos."
        )
